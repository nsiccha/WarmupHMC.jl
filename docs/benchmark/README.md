...
| `quoted_figures.jl` | the other half of that question: do the numbers **`RESULTS.md` quotes** still match the artifacts they were read out of? `RESULTS.md` is not a built page — not in `make.jl`'s `pages`, no `@eval` fence — so the docs build never executes a line of it and every figure in it is a static census. Exits 1 naming each disagreement. Run it after any regeneration, and after editing a number by hand. It also guards **absence claims** ("`cooperative_warmup_mcmc` was not measured"), whose falsifier is not a number moving but a results file appearing — the one class the recomputation checks are structurally blind to |
| `provenance_ordering.jl` | the one check asked of the **harnesses** rather than the artifacts: does any of them call `git_provenance()` (or its own `*_dirty` helper) inside its own `open(path, "w") do … end`? `open` truncates at open, so such a call sees the file it is writing and records `worktree_dirty = true` over a clean tree — a false claim under a correct table, invisible in the artifact until that file is tracked. Parses rather than greps, because the fix leaves both tokens in the file and only the block structure tells the shapes apart. Dependency-free; runs in `artifact-currency.yml` |
