...
function ace_summary_rows(d)
    ...
    for family in d["config"]["families"], arm in ace_arm_order()
        ...
        push!(out, (
            ...
            pooled_min_bulk_ess = pooled["min_ess_bulk"],
            pooled_bulk_ess_per_1000_grad =
                1000pooled["min_ess_bulk"] / pooled["total_gradient_evaluations"],
            pooled_min_tail_ess = pooled["min_ess_tail"],
            pooled_tail_ess_per_1000_grad =
                1000pooled["min_ess_tail"] / pooled["total_gradient_evaluations"],
            max_rank_rhat = pooled["max_rhat"],
        ))
    end
    out
end
...
function ace_raw_pooled_rows(d)
    ...
    [(
        ...
        arm = ace_arm_label(r["arm"]),
        n_chains = r["n_chains"],
        n_common_draws_per_chain = r["n_common_draws_per_chain"],
        divergences = r["total_divergent"],
        gradient_evaluations = r["total_gradient_evaluations"],
        min_bulk_ess = r["min_ess_bulk"],
        bulk_ess_per_1000_grad =
            1000r["min_ess_bulk"] / r["total_gradient_evaluations"],
        min_tail_ess = r["min_ess_tail"],
        tail_ess_per_1000_grad =
            1000r["min_ess_tail"] / r["total_gradient_evaluations"],
        max_rank_rhat = r["max_rhat"],
    ) for r in rows]
end
...
function ace_raw_chain_rows(d)
    ...
    [(
        ...
        gradients_per_draw = r["gradients_per_draw"],
        gradients_per_transition = r["gradients_per_transition"],
        min_bulk_ess = r["min_ess_bulk"],
        min_tail_ess = r["min_ess_tail"],
        bulk_ess_per_1000_grad = r["min_bulk_ess_per_1000_grad"],
        tail_ess_per_1000_grad = r["min_tail_ess_per_1000_grad"],
...
function ace_validation_rows(d)
    [(
        family = ace_family_label(family),
        n_points = v["n_points"],
        max_common_frame_abs = v["max_common_frame_abs"],
        max_coordinate_roundtrip_abs = v["max_coordinate_roundtrip_abs"],
        max_transport_logdensity_abs = v["max_transport_logdensity_abs"],
        max_gradient_fd_abs = v["max_gradient_fd_abs"],
...
function ace_comparison_rows(d)
    [begin
        ...
        (
            family = ace_family_label(family),
            median_proxy_reference_ratio = c["median_ratio"],
            ratio_range = ace_fmt_range(c["ratios"]; digits = 3),
            seeds_below_0_8 = string(c["below_0_8"], "/", length(c["ratios"])),
            verdict = c["verdict"],
        )
