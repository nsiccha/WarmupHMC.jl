...
# Distributions.logpdf(what::DynamicObject
...
function remaining_covariance(what::DynamicObject)
...
function kernels(what::Warmup)
  ...
  for window in what.warmup_windows
    ...
    # current_kernel = 
    # push!(rv, Diagonal(diag(scov)))
    # push!(rv, inv(Symmetric(gcov)))
    # push!(rv, Diagonal(1 ./ diag(gcov)))
    # push!(rv, Diagonal(sqrt.(diag(cov(gradients)) .\ diag(scov))))
    # push!(rv, Diagonal(sqrt.(diag(cov(gradients)) .\ diag(scov))))
    # println(eigen(scov).values)
    # println(1 ./ eigen(gcov).values)
    # println(eigen(scov).vectors' \ eigen(gcov).vectors)
  end
  return rv
end


function showcase(no_dimensions=10)
  sw = StanWarmup(1000)
  distribution = MvNormal(randn(no_dimensions), Diagonal(exp.(randn(no_dimensions))))
  ikernels = Warmup(IndependentKernel(distribution), sw).kernels
  mkernels = Warmup(MetropolisKernel(distribution, I), sw).kernels
  iconds = cond.(remaining_covariance.(ikernels))
  mconds = cond.(remaining_covariance.(mkernels))
