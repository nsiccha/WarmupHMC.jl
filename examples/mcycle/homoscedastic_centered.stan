...
functions {
  ...
  vector sqrt_spd_cov_exp_quad(vector[] x, real sdgp, vector lscale) {
    int NB = dims(x)[1];
    int D = dims(x)[2];
    int Dls = rows(lscale);
    vector[NB] out;
    if (Dls == 1) {
      // one dimensional or isotropic GP
      real constant = (sdgp) * sqrt(sqrt(2 * pi()) * lscale[1])^D;
      real neg_half_lscale2 = -0.25 * square(lscale[1]);
      for (m in 1:NB) {
        out[m] = constant * exp(neg_half_lscale2 * dot_self(x[m]));
      }
    } else {
      // multi-dimensional non-isotropic GP
      real constant = (sdgp) * sqrt(sqrt(2 * pi())^D * prod(lscale));
      vector[Dls] neg_half_lscale2 = -0.25 * square(lscale);
      for (m in 1:NB) {
        out[m] = constant * exp(dot_product(neg_half_lscale2, square(x[m])));
      }
    }
  ...
  vector gpa(matrix X, real sdgp, vector lscale, vector zgp, vector[] slambda) {
    vector[cols(X)] diag_spd = sqrt_spd_cov_exp_quad(slambda, sdgp, lscale);
    return X * (diag_spd .* zgp);
  }
  vector gpa(matrix X, vector gp) {
    return X * gp;
  }
}
data {
...
model {
  ...
  vector[N] mu = Intercept + rep_vector(0, N) + gpa(Xgp_1, gp_1);
  // initialize linear predictor term
  ...
  target += normal_lpdf(gp_1 | 0, sqrt_spd_cov_exp_quad(slambda_1, vsdgp_1[1], vlscale_1[1]));
